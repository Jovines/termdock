...
- **只有用户明确要求 Agent 测试时，才执行测试。**默认由用户自行测试，不主动运行
  单元测试、回归测试、浏览器交互测试或实机验收，也不反复询问是否需要测试。
- 本文件下文的测试与验收要求仅在用户明确要求测试时适用；提交、发版或修改特定
  模块本身不视为测试授权。未执行的测试不得声称通过。
- 部署所需的构建、服务重启和 health 健康检查仍按部署规则执行，不额外开展功能测试。
...
- **提交/发版例外**:用户明确要求提交或发版时,默认把当前工作区内所有人的改动
  ...
  **禁止** `git add -A` / `.` / `commit -a`,并在提交前完成构建；整体测试仅在用户明确要求时执行。
...
- 改完 `src/` 用 `termdock-deploy` skill 构建和部署；测试遵循上面的测试执行规则。遇他人 WIP 编译错时，按上面的可隔离检查与产物阻塞规则处理。
- **改动完成后自动部署到正式端口 9834**(`termdock-deploy` skill:
