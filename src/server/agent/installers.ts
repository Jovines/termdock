...
/** Shared budget for Termdock status emitters, in seconds. */
function hookTimeout(event: string): number {
  return event === 'session-end' ? 1 : 5;
}
  ...
  for (const [hookEvent, sentinel] of events) {
    ...
    if (ours !== null) {
      ...
      if (Array.isArray(list)) {
        const entry = list.find((entry) => markerCommand(entry, mark) === ours);
        const handler = entry.hooks.find((h: JsonObject) => h?.command === ours);
        if (handler.timeout !== hookTimeout(sentinel)) complete = false;
      }
    } else {
      complete = false;
    }
...
function hookMapInstall(file: string, agent: HookAgentSlug, events: Array<[string, string]>): void {
  ...
  for (const [hookEvent, sentinel] of events) {
    ...
    handler.timeout = hookTimeout(sentinel);
    kept.push({ hooks: [handler] });
...
function copilotHooksJson(): string {
  const hook = (event: string) => [{
    ...
    timeoutSec: hookTimeout(event),
  }];
  return JSON.stringify({
    version: 1,
    hooks: {
      sessionStart: hook('session-start'),
      userPromptSubmitted: hook('prompt-submit'),
      agentStop: hook('stop'),
...
export const TermdockPresence = async () => {
  ...
  const emit = (event, sessionID) => {
    ...
    return new Promise((resolve) => {
      const child = execFile(exe, [script, "agent-hook", "opencode", event], {
        timeout: event === "session-end" ? ${hookTimeout('session-end') * 1000} : ${hookTimeout('stop') * 1000},
        killSignal: "SIGKILL",
      }, () => resolve())
...
function emit(event: string): void {
  try {
    spawnSync(EXE, [SCRIPT, "agent-hook", "pi", event], {
...
function tomlHookBlock(hookEvent: string, command: string, sentinel: string): string {
  return `[[hooks]]\nevent = "${hookEvent}"\ncommand = '${command}'\ntimeout = ${hookTimeout(sentinel)}`;
}
  ...
  for (const [hookEvent, sentinel] of events) {
    ...
    const ok = ours.some(
      (s) => tomlSectionEvent(s) === hookEvent && tomlSectionCommand(s) === expected
        && Number(s.match(/^\s*timeout\s*=\s*(\d+)\s*(?:#.*)?$/m)?.[1]) === hookTimeout(sentinel),
    );
...
export function tomlHooksInstall(file: string, agent: HookAgentSlug, events: Array<[string, string]>): void {
  ...
  out += events
    .map(([hookEvent, sentinel]) => tomlHookBlock(hookEvent, hookCommand(agent, sentinel), sentinel))
    .join('\n\n');
  out += '\n';
  ...
  for (const [hookEvent, sentinel, matcher, timeout, status] of events) {
    ...
    if (ours !== undefined) {
      ...
      if (
        ...
        || (timeout ?? hookTimeout(sentinel)) !== handler.timeout
      ) complete = false;
    } else {
      complete = false;
    }
...
export function installPluginHooks(
  ...
  for (const [hookEvent, sentinel, matcher, timeout, status] of events) {
    ...
    const kept = list.filter((entry) => markerCommand(entry, mark) === null);
    const handler: JsonObject = { type: 'command', command };
    handler.timeout = timeout ?? hookTimeout(sentinel);
