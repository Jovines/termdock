...
import { RiTimerLine, RiInformationLine, RiAddLine, RiSubtractLine } from '@remixicon/react';
    ...
    <div className="p-4 space-y-4">
      <div>
        ...
        <select
          ...
          className="w-full px-4 py-3 text-base rounded-full bg-surface text-foreground min-h-[48px] appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-accent/40 transition-shadow"
        >
      ...
      {cleanupDurationPreset === 'custom' && (
        <div>
          ...
          <input
            ...
            placeholder="请输入分钟数"
            className="w-full px-4 py-3 text-base rounded-full bg-surface text-foreground min-h-[48px] focus:outline-none focus:ring-2 focus:ring-accent transition-shadow"
          />
      ...
      <div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-1 px-4 py-3 text-base border border-border/15 rounded-full bg-surface text-foreground min-h-[48px] hover:bg-surface-elevated transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            disabled={fontSize <= 8}
          >
            <RiSubtractLine className="w-5 h-5" />
            <span>减小</span>
          </button>
          <div className="flex-1 flex items-center justify-center px-4 py-3 text-base border border-border/15 rounded-full bg-surface text-foreground min-h-[48px]">
            <input
              id="mobile-fontsize"
              type="number"
              min="8"
      ...
      <div className="pt-2 border-t border-border/30">
        <button
          ...
          className={`w-full flex items-center justify-between px-4 py-3 text-sm rounded-full transition-colors ${
            showDebug ? 'bg-accent/15 text-accent' : 'bg-surface-2 hover:bg-surface-elevated'
          }`}
        >
          ...
          <span className={`text-xs px-3 py-0.5 rounded-full font-medium ${showDebug ? 'bg-accent text-accent-foreground' : 'bg-surface-elevated text-muted-foreground'}`}>
            {showDebug ? 'ON' : 'OFF'}
          </span>
        </button>
      </div>
